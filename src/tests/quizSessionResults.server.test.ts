...
describe('GET/v1/admin/quiz/{quizid}/session/{sessionid}/results', () => {
  describe('Error cases', () => {
    test('Quiz ID does not refer to a valid quiz', () => {
      const response = reqGetSessionResults(quiz.quizId + 1000, 100, user.token);
      expect(response).toStrictEqual({ error: 'Quiz ID does not refer to a valid quiz' });
    });
    test('Quiz ID does not refer to a quiz that this user owns', () => {
      const response = reqGetSessionResults(quiz.quizId, session.sessionId, user2.token);
      expect(response).toStrictEqual({ error: 'Quiz ID does not refer to a quiz that this user owns' });
    });
    test('Session Id does not refer to a valid session within this quiz', () => {
      const response = reqGetSessionResults(quiz.quizId, session.sessionId + 100, user.token);
    ...
    test('Session is not in FINAL_RESULTS state', () => {
      ...
      const response = reqGetSessionResults(quiz.quizId, session.sessionId, user.token);
      expect(response).toStrictEqual({ error: 'Session is not in FINAL_RESULTS state' });
    });
    test('Token is not a valid structure', () => {
      const response = reqGetSessionResults(quiz.quizId, session.sessionId);
    ...
    test('Provided token is a valid structure, but is not for a currently logged in session', () => {
      const response = reqGetSessionResults(quiz.quizId, session.sessionId, '123');
